...
export const CountdownBadge: React.FC<CountdownBadgeProps> = ({ deadline, showFull = false }) => {
  ...
  useEffect(() => {
    const update = () => setInfo(getDeadlineInfo(deadline));
    update();
  ...
  const renderCountdown = () => {
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        <span className="text-xs opacity-40 -mt-2">:</span>
        <span className={unitClass}>
          <span className={`${numClass} ${info.urgent ? 'animate-pulse' : ''}`}>{String(seconds).padStart(2, '0')}</span>
          <span className={labelClass}>秒</span>
        </span>
      </div>
    );
  };
